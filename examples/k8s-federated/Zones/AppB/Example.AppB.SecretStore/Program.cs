...
using Example.AppB.SecretStore;

// The appb-secretstore resource: this zone's secrets and leaf certificates. The SecretStore builder API used here is illustrative — the SecretStore area owns it. What this scaffold fixes is only that the resource is an ordinary executable (Program.cs), that orchestration is the opt-in in the csproj, and that its orchestration-facing facts live there.
...
builder.AddPolicy("certs/*", policy => policy.IssueLeaf(subject: key => $"{key}.appb.example.com", lifetime: TimeSpan.FromDays(30)));
builder.AddPolicy("*",       policy => policy.ReadableBy(application: "appb"));
